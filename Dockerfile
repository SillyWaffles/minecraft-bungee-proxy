...
WORKDIR /app
RUN apk add python3  # Instalar Python explícitamente
...
RUN echo '<html><body>OK</body></html>' > index.html  # Archivo para health check
RUN echo -e '#!/bin/sh\npython3 -m http.server 8080 &\njava -Xms256M -Xmx512M -jar velocity.jar' > start.sh && chmod +x start.sh
EXPOSE 25565
EXPOSE 8080
CMD ./start.sh
